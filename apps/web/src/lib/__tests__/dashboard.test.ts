...
describe('isHouseholdExpense', () => {
  ...
  it('収入 (income) は家計の支出ではない', () => {
    expect(isHouseholdExpense(makeTx({ type: 'income', category: 'salary' }))).toBe(false);
  });
  ...
  it('個人向け立替 (advanceToUserId != null) は家計の支出に含めない', () => {
    expect(
      isHouseholdExpense(makeTx({ type: 'advance', advanceToUserId: 'user-2' }))
    ).toBe(false);
  });
});
...
describe('calculateTransactionSummary', () => {
  ...
  it('世帯向け立替は支出合計に含め、個人向け立替は除外する', () => {
    const result = calculateTransactionSummary([
      ...
      makeTx({ id: 'c', type: 'advance', advanceToUserId: 'user-2', amount: 9999 }),
    ]);
...
describe('calculateExpenseCategoryBreakdown', () => {
  it('総額が 0 のときは空の内訳を返す', () => {
    expect(calculateExpenseCategoryBreakdown([])).toEqual({ total: 0, items: [] });
  });
